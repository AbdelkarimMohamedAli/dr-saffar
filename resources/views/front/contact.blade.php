...
">
        ...
        <!-- End Navbar Area -->
            ...
            <div class="contact-area ptb-100"<?php if(session('locale') == 'ar'){ ?> direction:rtl;<?php } ?>>
            <div class="container">
                <div class="contact-form-wrap">
                    <h3><?php if(session('locale') == 'ar'){ ?> Send Us a Message  <?php }else{ ?>  راسلنا الان  <?php } ?></h3>
                    <form  action="{{ route('contact.store') }}" method="POST">
                    @csrf
                        <div class="form-group">
                            <label><?php if(session('locale') == 'en'){ ?>Your Name  <?php }else{ ?>  الاسم <?php } ?></label>
                            <input type="text" name="name" id="name" class="form-control" required="" data-error="<?php if(session('locale') == 'en'){ ?> Please enter your name  <?php }else{ ?> من فضلك قم بكتابة الاسم  <?php } ?>" placeholder="<?php if(session('locale') == 'en'){ ?> Enter your name  <?php }else{ ?>  الاســــــم  <?php } ?>">
                            <div class="help-block with-errors"></div>
                        </div>

                        <div class="form-group">
                            <label><?php if(session('locale') == 'en'){ ?> Your Email <?php }else{ ?>  البريد الالكتروني <?php } ?></label>
                            <input type="email" name="email" id="email" class="form-control" required="" data-error="<?php if(session('locale') == 'en'){ ?> Please enter your email  <?php }else{ ?> من فضلك قم بكتابة البريد الالكتروني  <?php } ?>" placeholder="<?php if(session('locale') == 'en'){ ?>  Enter your email <?php }else{ ?> البريد الالكتروني  <?php } ?>">
                            <div class="help-block with-errors"></div>
                        </div>

                        <div class="form-group">
                            <label><?php if(session('locale') == 'en'){ ?>  Phone Number <?php }else{ ?>  رقم الهاتف <?php } ?></label>
                            <input type="number" name="phone" id="phone" class="form-control" required="" data-error="<?php if(session('locale') == 'en'){ ?> Please enter your phone number  <?php }else{ ?>  من فضلك قم بكتابة رقم الهاتف الخاص بك <?php } ?>" placeholder="<?php if(session('locale') == 'en'){ ?> Phone Number  <?php }else{ ?> رقم الهاتف  <?php } ?>">
                            <div class="help-block with-errors"></div>
                        </div>

                        <div class="form-group">
                            <label><?php if(session('locale') == 'en'){ ?>  Your Message  <?php }else{ ?>  رسالتك <?php } ?></label>
                            <textarea name="message" class="form-control" id="message" cols="30" rows="6" required="" data-error="   <?php if(session('locale') == 'ar'){ ?> Write your message  <?php }else{ ?>  اكتب رسالتك <?php } ?>" placeholder="<?php if(session('locale') == 'en'){ ?> Type your message  <?php }else{ ?>    من فضلك اكتب رسالتك  <?php } ?>"></textarea>
                            <div class="help-block with-errors"></div>
                        </div>

                        <button type="submit" class="default-btn"><?php if(session('locale') == 'ar'){ ?>  Send Message <?php }else{ ?> ارسل الان  <?php } ?> </button>
